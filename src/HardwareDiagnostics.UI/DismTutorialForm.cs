...
{
    public class DismTutorialForm : Form
    {
        ...
        public DismTutorialForm()
        {
            Text = "DISM 终极教程 V3 - 目录导航版";
        ...
        private void InitializeComponents()
        {
            ...
            searchBox.TextChanged += (s, e) => {
                hintLabel.Visible = string.IsNullOrEmpty(searchBox.Text);
                SearchBox_TextChanged(s, e);
            };
            
            searchPanel.Controls.Add(hintLabel);
            ...
            var rightHeaderLabel = new Label
            {
                Text = "📝 内容",
                Font = new Font("Microsoft YaHei", 12F, FontStyle.Bold),
                Dock = DockStyle.Fill,
            ...
            rightPanel.Controls.Add(_contentTextBox);
        ...
        {
            _sections = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);

            // 简介
            _sections["简介"] = GetIntroduction();

            // 第一章
            _sections["第一章：基础概念"] = GetChapter1();

            // 第二章
            _sections["第二章：快速入门"] = GetChapter2();

            // 第三章
            _sections["第三章：系统健康检查"] = GetChapter3();

            // 第四章
            _sections["第四章：组件存储清理"] = GetChapter4();

            // 第五章
            _sections["第五章：Windows功能管理"] = GetChapter5();

            // 第六章
            _sections["第六章：包管理"] = GetChapter6();

            // 第七章
            _sections["第七章：驱动管理"] = GetChapter7();

            // 第八章
            _sections["第八章：映像文件操作"] = GetChapter8();

            // 第九章
            _sections["第九章：故障排除"] = GetChapter9();

            // 附录
            _sections["附录：快速参考"] = GetAppendix();
        ...
        {
            // 根节点
            var root = new TreeNode("📖 DISM 终极教程 V3", 0, 0);
            root.Tag = "简介";
            ...
            {
                var node = new TreeNode(chapter, 1, 1);
                node.Tag = chapter;
                root.Nodes.Add(node);
            }

            // 附录
            var appendix = new TreeNode("📋 附录：快速参考", 2, 2);
            appendix.Tag = "附录：快速参考";
            root.Nodes.Add(appendix);
        ...
        {
            if (e.Node?.Tag is string key && _sections.ContainsKey(key))
            {
                ShowContent(key);
            }
        }
        ...
        {
            if (_sections.TryGetValue(key, out string content))
            {
...
【它能做什么？】
  ✓ 检查并修复系统文件损坏（不用重装系统！）
  ✓ 清理 WinSxS 组件存储释放磁盘空间
  ✓ 启用/禁用 Windows 功能（如 .NET Framework）
  ✓ 安装/卸载更新包和驱动程序
  ✓ 管理 Windows 映像文件（WIM/VHD/VHDX）
...
命令：
  DISM /Online /Get-PackageInfo /PackageName:Package_for_KB5028185~...
...
命令：
  DISM /Online /Remove-Package /PackageName:Package_for_KB5028185~...
